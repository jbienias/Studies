...
namespace Aproksymacja {
    public class GameStateGenerator {
        ...
        public void generateMatrixToFile(string filename) {
            ...
            using (StreamWriter sw = new StreamWriter(filename)) {
                ...
                foreach (var state in allStates) {
                    if (state.win)
                        sw.WriteLine(state.index + " " + state.index + " " + 1.0);
                    else
                        sw.WriteLine(state.index + " " + state.index + " " + -1.0);
                    foreach (var subState in state.equation) {
                        sw.WriteLine(state.index + " " + subState.index + " " + (double)state.countSameStates(subState, dice) / dice.probabilitySum);
                    }
                }
            }
            customCulture.NumberFormat.NumberDecimalSeparator = ",";
