...
VectorXi loadVectorNonZero(string filename) {
	fstream file(filename, ios_base::in);
	int rows;
	int columns;
	file >> rows;
	file >> columns;
	VectorXi vector(rows);
	for (int i = 0; i < rows; i++) {
		file >> vector(i);
	}
	return vector;
...
void info() {
	...
	printf("\n===================== Gauss SparseLU =====================\n");
	//Prepare
	VectorXi vectorNonZero = loadVectorNonZero(vectorNonZeroInput);
	...
	printf("\n===================== Gauss Seidel Eigen =====================\n");
	//Prepare
	vectorNonZero = loadVectorNonZero(vectorNonZeroInput);
